...
const problems = [
    ...
    },
    {
        title: 'EXERCISE 3.4',
        body: `
            แก้ไขฟังก์ชัน \`findMinMax(input)\`<br><br>

            กำหนดให้ \`input\` เป็น array ของตัวเลข  
            ให้หาตัวเลขที่มากที่สุดและน้อยที่สุดใน \`input\`
        `,
        testcases: [
            { input: [1, 2, 3, 4, 5], expect: { min: 1, max: 5 }, result: null },
            { input: [13, 28, 0, 201], expect: { min: 0, max: 201 }, result: null },
            { input: [9, 2, 11, 3, 8], expect: { min: 2, max: 11 }, result: null },
            { input: [4, 1, 5, 2, -3, 9, 0], expect: { min: -3, max: 9 }, result: null },
        ],
        run: findMinMax
    },
    {
        title: 'EXERCISE 3.5',
        body: `
            แก้ไขฟังก์ชัน \`findBMI(input)\`<br><br>
            ...
            ให้คำนวณดัชนีมวลการ (Body Mass Index, bmi) ของแต่ละคน  
            โดย bmi = น้ำหนักในหน่วยกิโลกรัม / ความสูงในหน่วยเมตรยกกำลังสอง (ให้ตอบทศนิยมสองตำแหน่ง)
            <br><br>

            <details>
                <summary>Help</summary>
                <li><a href="https://www.w3schools.com/jsref/jsref_tofixed.asp">toFixed()</a></li>
            </details>
        `,
        testcases: [
            {
                input: [
                    { name: "Aariz Bennett", height: 180, weight: 72 },
                    { name: "Najma Shaffer", height: 162, weight: 74 },
                    { name: "Jill Schmitt", height: 145, weight: 58 },
                    { name: "Anita Rose", height: 174, weight: 60 },
                ],
                expect: [
                    {
                        "name": "Aariz Bennett",
